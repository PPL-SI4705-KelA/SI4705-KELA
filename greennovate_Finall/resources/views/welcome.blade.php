...
@section('title', 'Greennovate – Pulihkan Lahan, Hijaukan Masa Depan')
                ...
                <div class="relative rounded-2xl overflow-hidden shadow-xl">
                    ...
                    <div class="absolute bottom-4 left-4 right-4 flex gap-2">
                        <div class="animate-float bg-white/95 backdrop-blur-sm rounded-xl px-3 py-2 flex-1 text-center shadow-md">
                            ...
                            <div class="text-base font-extrabold text-gray-900">{{ number_format($stats['pohon_ditanam'] ?: 0) }}</div>
                        </div>
                        <div class="animate-float bg-white/95 backdrop-blur-sm rounded-xl px-3 py-2 flex-1 text-center shadow-md" style="animation-delay:0.3s">
                            ...
                            <div class="text-base font-extrabold text-gray-900">{{ $stats['total_lokasi'] }} Ha</div>
                        </div>
                        <div class="animate-float bg-white/95 backdrop-blur-sm rounded-xl px-3 py-2 flex-1 text-center shadow-md" style="animation-delay:0.6s">
                            ...
                            <div class="text-base font-extrabold text-gray-900">{{ number_format($stats['total_relawan']) }}</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="max-w-6xl mx-auto px-6">
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="stat-card bg-white rounded-2xl p-6 border border-gray-100 shadow-sm reveal">
                ...
                <div class="text-2xl font-extrabold text-gray-900 mb-0.5">{{ number_format($stats['pohon_ditanam'] ?: 0) }}</div>
            ...
            <div class="stat-card bg-white rounded-2xl p-6 border border-gray-100 shadow-sm reveal" style="transition-delay:.08s">
                ...
                <div class="text-2xl font-extrabold text-gray-900 mb-0.5">{{ number_format($stats['total_lokasi']) }}</div>
            ...
            <div class="stat-card bg-white rounded-2xl p-6 border border-gray-100 shadow-sm reveal" style="transition-delay:.16s">
                ...
                <div class="text-2xl font-extrabold text-gray-900 mb-0.5">{{ number_format($stats['total_relawan']) }}</div>
            ...
            <div class="stat-card bg-white rounded-2xl p-6 border border-gray-100 shadow-sm reveal" style="transition-delay:.24s">
                ...
                <div class="text-2xl font-extrabold text-gray-900 mb-0.5">{{ number_format($stats['total_program'] ?: 0) }}</div>
...
<section id="lokasi" class="py-14 bg-white">
    <div class="max-w-6xl mx-auto px-6">
        <div class="flex items-end justify-between mb-8 reveal">
            ...
            @auth
            <a href="{{ route('kegiatan.index') }}" class="text-sm font-semibold text-[#0D8B41] hover:underline flex items-center gap-1">
                Lihat semua
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @else
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                @foreach($lokasi_terbaru as $i => $lokasi)
                <a href="{{ route('kegiatan.index', ['lokasi' => $lokasi->id]) }}" class="block feature-card bg-[#f7f7f3] rounded-2xl overflow-hidden border border-gray-100 reveal hover:shadow-md transition-shadow" data-delay="{{ $i * 100 }}">
                    ...
                    <div class="p-5">
                        <h3 class="font-bold text-gray-900 text-base mb-1">{{ $lokasi->nama }}</h3>
                        @if($lokasi->alamat)
                        ...
                        @endif
                        @if($lokasi->deskripsi)
                        <p class="text-xs text-gray-500 leading-relaxed line-clamp-2">{{ $lokasi->deskripsi }}</p>
                        @endif
                    </div>
                </a>
                @endforeach
