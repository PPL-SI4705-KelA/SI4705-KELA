...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function appendMessage(msg) {
        ...
        wrapper.className = `flex ${isAdmin ? 'justify-end' : 'justify-start'} msg-menu-container group relative`;
        
        const bubbleContainer = document.createElement('div');
        bubbleContainer.className = `max-w-[75%] flex flex-col gap-1 relative ${isAdmin ? 'items-end' : 'items-start'}`;
        ...
        if (isAdmin) {
            ...
            menuBtn.className = 'absolute -left-6 top-5 text-gray-400 hover:text-gray-600 px-2 opacity-0 group-hover:opacity-100 transition z-10';
            menuBtn.onclick = (e) => { e.stopPropagation(); toggleMenu(msg.id); };
            
            const dropdown = document.createElement('div');
            dropdown.id = `msg-menu-${msg.id}`;
            dropdown.className = 'msg-menu-dropdown hidden absolute -left-32 top-10 bg-white border border-gray-200 shadow-md rounded-md text-sm z-20 w-28 text-gray-700 overflow-hidden';
            ...
            bubbleContainer.appendChild(menuBtn);
            bubbleContainer.appendChild(dropdown);
        }
