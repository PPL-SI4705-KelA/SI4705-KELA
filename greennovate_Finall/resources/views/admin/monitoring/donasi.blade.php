...
<div class="flex flex-col gap-6">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        @else
            <form action="{{ route('admin.donasi.terima-massal') }}" method="POST" id="bulk-accept-form" onsubmit="return prepareBulkSubmit(event)">
                @csrf
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                @if($d->bukti_dokumentasi)
                                    <button type="button" id="btn-lihat-{{ $d->id }}" onclick="openImageModal('{{ asset('storage/' . $d->bukti_dokumentasi) }}')" class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-[#0D8B41] bg-green-50 rounded-lg hover:bg-green-100 transition-colors border border-green-200">
                                ...
                                @if(strtolower($d->status) === 'pending' || strtolower($d->status) === 'menunggu_verifikasi')
                                    <div class="flex gap-2">
                                        <form action="{{ route('admin.donasi.terima', $d->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menerima donasi ini?');">
                                            @csrf
                                            <button type="submit" id="btn-terima-{{ $d->id }}" class="bg-green-600 hover:bg-green-700 text-white text-xs font-bold py-1.5 px-3 rounded-lg shadow-sm">
                                                Terima
                                            </button>
                                        </form>
                                        <form action="{{ route('admin.donasi.tolak', $d->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menolak donasi ini?');">
                                            @csrf
                                            <button type="submit" id="btn-tolak-{{ $d->id }}" class="bg-red-600 hover:bg-red-700 text-white text-xs font-bold py-1.5 px-3 rounded-lg shadow-sm">
                                                Tolak
                                            </button>
                                        </form>
                                    </div>
...
function prepareBulkSubmit(e) {
    if(!confirm('Yakin ingin menerima donasi yang dipilih?')) {
        e.preventDefault();
        return false;
    }
    const checkboxes = document.querySelectorAll('.bulk-checkbox:checked');
