...
@section('title', 'Peserta Kegiatan – Greennovate')
@section('page-title', 'Peserta Kegiatan')
@section('page-subtitle', 'Daftar partisipan aktif di setiap kegiatan')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select id="kegiatan-select"
                    ...
                    <option value="" disabled {{ !$selectedKegiatan ? 'selected' : '' }}>-- Pilih Kegiatan --</option>
            ...
            {{-- Select Status --}}
            <div>
                <form method="GET" action="{{ $selectedKegiatan ? route('admin.kegiatan.peserta', $selectedKegiatan->id) : route('admin.peserta.index') }}" id="filter-form">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 class="font-bold text-gray-800 text-sm">
                @if($selectedKegiatan)
                    Peserta untuk: <span class="text-[#0D8B41]">{{ $selectedKegiatan->nama }}</span>
                @else
                    Silakan Pilih Kegiatan Terlebih Dahulu
                @endif
            </h2>
            <span class="text-xs text-gray-400">Total: {{ $selectedKegiatan ? $peserta->total() : 0 }}</span>
        </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-50 text-sm">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 text-gray-600 font-medium">{{ $p->kegiatan ? $p->kegiatan->nama : 'N/A' }}</td>
