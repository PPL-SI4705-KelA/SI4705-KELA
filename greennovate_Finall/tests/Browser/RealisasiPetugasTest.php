...
class RealisasiPetugasTest extends DuskTestCase
{
    ...
    private function prepareDummyData()
    {
        ...
        $kegiatanBerau = Kegiatan::where('nama', 'Aksi Tanam Pohon Berau')->first();
        
        if (!$kegiatanBerau) {
            $lokasiBerau = \App\Models\LokasiLahan::firstOrCreate(
                ['nama' => 'Hutan Lindung Berau'],
                [
                    'alamat' => 'Kabupaten Berau',
                    'deskripsi' => 'Hutan Lindung'
                ]
            );

            $kegiatanBerau = Kegiatan::updateOrCreate(
                ['slug' => 'aksi-tanam-pohon-berau'],
                [
                    'nama' => 'Aksi Tanam Pohon Berau',
                    'deskripsi' => 'Kegiatan penanaman',
                    'lokasi' => 'Berau',
                    'lokasi_lahan_id' => $lokasiBerau->id,
                    'petugas_id' => User::where('email', $this->petugasEmail)->first()->id ?? 1,
                    'tanggal' => now(),
                    'target_donasi' => 100000,
                    'target_pohon' => 3000,
        ...
        $pohonDurian = JenisPohon::updateOrCreate(
            ['nama' => 'Durian (Durio zibethinus)'],
            [
                'nama_latin' => 'Durio zibethinus',
                'deskripsi' => 'Pohon durian',
                'harga' => 115000,
                'kategori_pohon_id' => \App\Models\KategoriPohon::first()->id ?? 1,
            ]
        );

        // Buat Transaksi Sukses untuk Pohon Durian
        Pembelian::updateOrCreate(
            ['kode_transaksi' => 'GNV-20260622-2895'],
            [
                'user_id' => User::where('email', $this->userEmail)->first()->id ?? 1,
                'nama_item' => 'Durian (Durio zibethinus) - Lahan: Hutan Lindung Berau',
                'jumlah_pohon' => 1,
                'total_harga' => 115000,
                'status' => 'Sukses',
            ]
        ...
        return [
            ...
            'pohonA' => $pohonA,
            'pohonB' => $pohonB,
            'kegiatanBerau' => $kegiatanBerau,
            'pohonDurian' => $pohonDurian,
        ];
    }
    ...
    /**
     * TC-07: Menginput pohon yang sudah terverifikasi (Durian - Hutan Lindung Berau)
     */
    public function testPetugasInputPohonTerverifikasi()
    {
        $data = $this->prepareDummyData();
        ...
        $this->browse(function (Browser $browser) use ($data) {
            $this->loginAsUser($browser, $this->petugasEmail)
                 ->visit('/petugas/realisasi')
                 ->select('kegiatan_id', (string)$data['kegiatanBerau']->id)
                 ->select('jenis_pohon_id', (string)$data['pohonDurian']->id)
                 ->type('jumlah_tertanam', '1')
                 ->press('Simpan Realisasi')
                 ->waitForText('Realisasi penanaman berhasil disimpan', 10)
                 ->logout();
        });
    }
}
