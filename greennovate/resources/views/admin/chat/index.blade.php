...
@section('title', 'Pesan Masuk – Admin Greennovate')
@section('page-title', 'Pesan Masuk')
@section('page-subtitle', 'Kelola percakapan dari pengguna')
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach($conversations as $conv)
                        <tr class="hover:bg-gray-50 transition">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="font-medium text-gray-900 flex items-center gap-2">
                                            {{ $conv->user->name }}
                                            @if($conv->unread_count > 0)
                                                <span class="bg-red-100 text-red-700 text-[10px] font-bold px-2 py-0.5 rounded-full">{{ $conv->unread_count }} Baru</span>
                                            @endif
                                        </div>
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $conv->status === 'open' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                    {{ ucfirst($conv->status) }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('admin.chat.show', $conv->id) }}" class="inline-flex items-center gap-1 bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 px-3 py-1.5 rounded-lg text-sm font-medium transition shadow-sm">
                                    Buka Pesan
                                </a>
                            </td>
                        </tr>
