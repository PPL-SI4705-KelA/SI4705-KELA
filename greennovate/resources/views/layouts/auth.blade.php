            ...
            <div class="flex items-center gap-4 text-sm font-medium">
                @guest
                    ...
                    <a href="{{ route('register') }}"
                        class="bg-[#1b7b43] text-white px-4 py-2 rounded-full hover:bg-green-700 transition">{{ __('Join Activity') }}</a>
                @else
                    <a href="{{ route('dashboard') }}" class="text-gray-500 hover:text-black">{{ __('Dashboard') }}</a>
                    <a href="{{ route('profile.edit') }}" class="text-gray-500 hover:text-black">{{ __('Profile') }}</a>
                    <form method="POST" action="{{ route('logout') }}" class="inline">
                        @csrf
                        <button type="submit"
                            class="bg-red-50 text-red-600 px-4 py-2 rounded-full hover:bg-red-100 transition">{{ __('Logout') }}</button>
                    </form>
                @endguest
            </div>
        </header>
