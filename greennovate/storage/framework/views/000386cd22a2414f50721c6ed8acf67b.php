<?php $__env->startSection('title', 'Dashboard Petugas - Greennovate'); ?>
<?php $__env->startSection('header', 'Kegiatan Saya'); ?>
...
<div class="max-w-6xl">
    ...
    <div class="bg-gradient-to-r from-[#1a8245] to-[#15803d] p-6 rounded-2xl shadow-md mb-8 text-white relative overflow-hidden">
        ...
        <div class="relative z-10">
            <h1 class="text-2xl font-bold mb-1"><?php echo e($greeting); ?>, <?php echo e($user->name); ?> 👋</h1>
            <p class="text-green-100 text-sm">
                <?php echo e(now()->translatedFormat('l, d F Y • H:i')); ?> WIB
            </p>
            <div class="mt-3 flex items-center gap-2 text-sm text-green-100">
                ...
                Petugas Lapangan · <?php echo e($user->city ?? 'Indonesia'); ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
        <?php $__empty_1 = true; $__currentLoopData = $kegiatanAktif; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kegiatan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <?php
            ...
            <div class="bg-white border rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300 <?php echo e($isLowProgress ? 'border-orange-200' : 'border-gray-100'); ?>"
                 id="kegiatan-card-<?php echo e($kegiatan->id); ?>">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex-1 min-w-0">
                        <h3 class="font-bold text-gray-900 text-[15px] mb-1.5 truncate" title="<?php echo e($kegiatan->nama); ?>">
                            <?php echo e(Str::limit($kegiatan->nama, 50)); ?>
                        ...
                        <div class="flex items-center gap-1.5 text-[13px] text-gray-500 mb-1">
                            ...
                            <span class="truncate"><?php echo e($kegiatan->lokasiLahan?->alamat ?? 'Lokasi tidak ditentukan'); ?></span>
                        </div>
                    ...
                    <div class="flex items-center gap-1.5 <?php echo e($statusConfig['bg']); ?> <?php echo e($statusConfig['text']); ?> text-[11px] font-bold px-3 py-1.5 rounded-full flex-shrink-0 ml-3">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="<?php echo e($statusConfig['icon']); ?>"/>
                        </svg>
                ...
                <div class="mb-5 mt-4">
                    <div class="flex justify-between items-end mb-2">
                        <div class="flex items-center gap-1.5 text-[13px] font-medium text-gray-500">
                            <svg class="w-4 h-4" style="color: <?php echo e($progressColor); ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                            </svg>
                            Pohon Ditanam
                        </div>
                        <div class="text-[14px] font-bold text-gray-900" id="progress-text-<?php echo e($kegiatan->id); ?>">
                            <?php echo e(number_format($kegiatan->realisasi_pohon, 0, ',', '.')); ?>

                            <span class="text-xs font-medium text-gray-400">/ <?php echo e(number_format($kegiatan->target_pohon, 0, ',', '.')); ?></span>
                        </div>
                    </div>
                    <div class="w-full rounded-full h-2.5 mb-1.5 overflow-hidden" style="background: <?php echo e($progressBg); ?>">
                        <div class="h-2.5 rounded-full transition-all duration-700 ease-out" id="progress-bar-<?php echo e($kegiatan->id); ?>"
                             style="width: <?php echo e($percentage); ?>%; background-color: <?php echo e($progressColor); ?>"></div>
                    </div>
                    <div class="text-[11px] font-medium <?php echo e($isLowProgress ? 'text-orange-500' : 'text-gray-400'); ?>" id="progress-pct-<?php echo e($kegiatan->id); ?>">
                        <?php echo e($percentage); ?>% tercapai
                        <?php if($isLowProgress): ?>
                ...
                <div class="flex gap-3">
                    <a href="<?php echo e(route('petugas.realisasi', ['kegiatan_id' => $kegiatan->id])); ?>"
                    ...
                    <button type="button" onclick="openDokumentasiModal(<?php echo e($kegiatan->id); ?>, '<?php echo e(addslashes($kegiatan->nama)); ?>')" class="px-5 border border-gray-200 text-gray-600 hover:bg-gray-50 hover:text-gray-900 text-[13px] font-semibold rounded-xl flex items-center justify-center gap-2 transition-colors min-h-[44px]">
                        <svg class="w-[14px] h-[14px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
            ...
            <div class="col-span-full py-16 text-center bg-white border border-gray-100 rounded-2xl shadow-sm">
                ...
                <a href="<?php echo e(route('petugas.semua-kegiatan')); ?>"
...
<script>
    ...
    fetch('<?php echo e(route("petugas.api.jenis-pohon")); ?>')
        .then(r => r.json())
    ...
    document.getElementById('realisasiForm').addEventListener('submit', async function(e) {
        ...
        try {
            const url = '<?php echo e(url("petugas/api/kegiatan")); ?>/' + kegiatanId + '/realisasi';
    ...
    document.getElementById('dokumentasiForm').addEventListener('submit', async function(e) {
        ...
        try {
            const url = '<?php echo e(url("petugas/api/kegiatan")); ?>/' + kegiatanId + '/dokumentasi';
            const res = await fetch(url, {
                method: 'POST',
                headers: { 
    ...
    setInterval(() => {
        fetch('<?php echo e(route("petugas.api.dashboard")); ?>')
            .then(r => r.json())
            .then(activities => {
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.petugas', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\rizky\Documents\GitHub\SI4705-KELA\greennovate\resources\views/petugas/dashboard.blade.php ENDPATH**/ ?>